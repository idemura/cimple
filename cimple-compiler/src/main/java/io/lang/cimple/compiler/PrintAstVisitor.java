...
public class PrintAstVisitor extends AstVisitor {
  ...
  protected void visit(AstFunction node) {
    printEntity("FUNCTION", node.name(), node.resultType());
    output.indent();
  ...
  protected void visit(AstFunctionType node) {
    ...
    printEntity("TYPE FUNCTION", node.name(), function.resultType());
    output.indent();
  ...
  protected void visit(AstInterfaceType node) {
    output.writeLine("TYPE INTERFACE %s".formatted(node.name()));
    output.indent();
    node.acceptChildren(this);
    output.unindent();
    output.writeLine("END");
  ...
  protected void visit(AstStructType node) {
    output.writeLine("TYPE STRUCT %s".formatted(node.name()));
    output.indent();
    node.acceptChildren(this);
    output.unindent();
    output.writeLine("END");
  ...
  protected void visit(AstEnumType node) {
    ...
    for (var variant : node.variants()) {
      if (variant.expression() == null) {
        ...
        output.writeLine("VARIANT %s(%s)".formatted(variant.tag(), variant.expression()));
      }
    }
    output.unindent();
    output.writeLine("END");
